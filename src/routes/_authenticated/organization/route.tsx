...
export const Route = createFileRoute("/_authenticated/organization")({
  beforeLoad: async () => {
    ...
    if (profile?.primary_role !== "organization") throw redirect({ to: "/" });
  },
...
type OrgGate = { id: string; verification_status: string; rejection_reason: string | null } | null;

async function fetchGateData(): Promise<{ profile: ProfileGate | null; org: OrgGate }> {
  const { data: userData } = await supabase.auth.getUser();
  if (!userData.user) return { profile: null, org: null };
  const { data: profile } = await supabase
    .from("profiles")
    .select("account_status, account_rejection_reason")
    .eq("id", userData.user.id)
    .maybeSingle();
  if (profile?.account_status !== "approved") return { profile: profile as ProfileGate, org: null };
  const { data: org } = await supabase
    ...
    .limit(1)
    .maybeSingle();
  return { profile: profile as ProfileGate, org: org as OrgGate };
}
...
function OrganizationShell() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ["organization-gate"], queryFn: fetchGateData });
        ...
        <div className="w-full max-w-md rounded-3xl border border-border bg-card p-8 text-center">
          ...
          {rejected ? (
            ...
            <>
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Thanks for registering your Organization. An admin needs to approve your account before you can
  ...
  return (
    <div className="min-h-dvh bg-muted/30">
      <div className="md:hidden sticky top-0 z-40 flex h-14 items-center justify-between border-b border-border bg-background/90 backdrop-blur px-4">
        <Link to="/organization"><Logo className="h-7" /></Link>
