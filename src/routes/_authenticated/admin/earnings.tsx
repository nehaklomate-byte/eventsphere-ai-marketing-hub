...
const FEATURE_LABEL: Record<string, string> = {
  profile_activation: "Profile link activation", subscription_monthly: "Pro plan — monthly", subscription_annual: "Pro plan — annual",
};
...
function ProfileRevenueTable({ rows, isLoading }: { rows: ProfileRevenueRow[]; isLoading: boolean }) {
  if (isLoading) return <LoadingBlock />;
  if (rows.length === 0) return <EmptyBlock text="No profile-link or subscription payments yet." />;
  return (
    <div className="overflow-x-auto rounded-2xl border border-border bg-card">
      <div className="flex justify-end p-3">
        <button onClick={() => downloadCsv("profile-revenue.csv", rows as never)} className="text-xs font-semibold text-brand-violet hover:underline">Download CSV</button>
      </div>
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-border text-left text-xs font-semibold uppercase tracking-wide text-muted-foreground">
        ...
        <tbody>
          {rows.map((r) => (
            <tr key={r.id} className="border-b border-border last:border-0">
              <td className="px-5 py-3 capitalize"><span className={`rounded-full px-2.5 py-0.5 text-[11px] font-semibold ${SOURCE_STYLE[r.role]}`}>{r.role}</span></td>
              <td className="px-5 py-3 font-medium">{r.entity_name}</td>
              <td className="px-5 py-3 text-muted-foreground">{FEATURE_LABEL[r.feature_type] ?? r.feature_type}</td>
              <td className="px-5 py-3 font-semibold">{money(r.amount)}</td>
              <td className="px-5 py-3">
                {r.status === "paid" ? (
                  ...
                  <span className="rounded-full bg-amber-100 text-amber-800 dark:bg-amber-950/40 dark:text-amber-300 px-2.5 py-0.5 text-[11px] font-semibold capitalize">{r.status}</span>
                )}
              </td>
              <td className="px-5 py-3 text-muted-foreground font-mono text-xs">{r.razorpay_payment_id || "—"}</td>
              <td className="px-5 py-3 text-muted-foreground">{new Date(r.created_at).toLocaleString("en-IN")}</td>
              <td className="px-5 py-3">
                {r.status === "paid" && (
                  <Link to="/receipt/$type/$id" params={{ type: "profile", id: r.id }} target="_blank"
                    className="inline-flex items-center gap-1 text-xs font-semibold text-brand-violet hover:underline">
                    <Download className="h-3.5 w-3.5" /> View
                  </Link>
                )}
