...
export const Route = createFileRoute("/_authenticated/venue")({
  beforeLoad: async () => {
    ...
    const { data: profile, error } = await supabase
      .from("profiles")
      .select("primary_role, approval_status")
    ...
    <div className="min-h-dvh bg-muted/30">
      ...
      <div className="mx-auto flex max-w-[1400px]">
        <aside
          ...
          } md:translate-x-0 fixed md:sticky top-0 z-50 md:z-30 h-dvh w-72 shrink-0 border-r border-border bg-background/95 backdrop-blur transition-transform`}
        >
          ...
          <nav className="flex flex-col gap-0.5 p-3">
              ...
              return (
                <Link
                  ...
                  className={`group flex items-center justify-between rounded-xl px-3 py-2.5 text-sm font-medium transition ${
                    active
                      ? "bg-gradient-to-r from-brand-violet/15 to-secondary/10 text-foreground shadow-sm"
            ...
            <button
              onClick={signOut}
              className="mt-2 flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-muted-foreground hover:bg-accent hover:text-foreground"
            >
