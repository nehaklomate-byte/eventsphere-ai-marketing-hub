...
import {
  fetchMyHalls, fetchHallPostings, createHallJobPosting, closeHallPosting, fetchApplicationsForHallPosting,
...
export const Route = createFileRoute("/_authenticated/venue/jobs")({
  head: () => ({ meta: [{ title: "Job Board — EventOrbit AI" }, { name: "robots", content: "noindex" }] }),
  component: VenueJobsPage,
});
...
function VenueJobsPage() {
  const { user } = useSession();
  const qc = useQueryClient();
  const { data: halls = [] } = useQuery({ queryKey: ["my-halls"], queryFn: fetchMyHalls, enabled: !!user?.id });
  const [hallId, setHallId] = useState<string>("");
  const activeHallId = hallId || halls[0]?.id || "";

  const { data: postings, isLoading } = useQuery({
    queryKey: ["hall-postings", activeHallId],
    queryFn: () => fetchHallPostings(activeHallId),
  ...
  async function handleClose(id: string) {
    ...
    try {
      await closeHallPosting(id);
      await qc.invalidateQueries({ queryKey: ["hall-postings", activeHallId] });
    } catch (err) {
    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Post on-ground staffing needs for events at your venue — waiters, cleaners, technicians and more —
            and workers on EventOrbit apply directly. Accepting an applicant creates their task automatically.
          </p>
        </div>
        <div className="flex items-center gap-3">
          {halls.length > 1 && (
            <select value={activeHallId} onChange={(e) => setHallId(e.target.value)}
      ...
      {!activeHallId && (
        <div className="rounded-2xl border border-dashed border-border bg-card/40 p-8 text-center text-sm text-muted-foreground">
          Add your venue in Venue Profile first — job postings are linked to a venue.
        </div>
      )}

      <div className="space-y-3">
        {isLoading && <p className="text-sm text-muted-foreground">Loading postings…</p>}
        {!isLoading && activeHallId && (postings?.length ?? 0) === 0 && (
        ...
        {(postings ?? []).map((p) => (
          <div key={p.id} className="rounded-2xl border border-border bg-card overflow-hidden">
            ...
            {expanded === p.id && <ApplicationsPanel postingId={p.id} hallId={activeHallId} />}
          </div>
        ))}
      </div>
      ...
      {open && activeHallId && (
        <NewPostingModal
          ...
          onDone={() => { setOpen(false); qc.invalidateQueries({ queryKey: ["hall-postings", activeHallId] }); }}
        />
      )}
    </div>
  );
...
function ApplicationsPanel({ postingId, hallId }: { postingId: string; hallId: string }) {
  const qc = useQueryClient();
  const { data: apps, isLoading } = useQuery({
    queryKey: ["hall-posting-applications", postingId],
    queryFn: () => fetchApplicationsForHallPosting(postingId),
  });
  ...
  async function act(action: "shortlist" | "reject" | "accept", app: JobApplication) {
    try {
      ...
      await qc.invalidateQueries({ queryKey: ["hall-posting-applications", postingId] });
      await qc.invalidateQueries({ queryKey: ["hall-postings", hallId] });
...
function NewPostingModal({ hallId, onClose, onDone }: { hallId: string; onClose: () => void; onDone: () => void }) {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      await createHallJobPosting(hallId, {
        event_id: null,
        ...
        <form onSubmit={submit} className="mt-4 space-y-3">
          <Field label="Title *">
            <input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })}
              placeholder="e.g. 4 cleaners for Sunday wedding" className={cls} />
          </Field>
